...
const svg_files_folder = './icons-svg',
    dest_folder = '../src/components/shared/icons/',
...
gulp.task('icons_components', function () {
    return gulp
            ...
            svgmin({
                ...
                plugins: [
                    ...
                    'removeEmptyContainers',
                    {
                        ...
                        params: {
                            attrs: [
                                '(filter|fill|stroke|fill-rule|stroke-linecap|stroke-linejoin|stroke-width)',
                                'svg:(width|height)',
                            ],
                        },
                    },
        ...
        .pipe(replace(/<\?xml[^>]*\?>/, ''))  // Added to remove XML declaration
        // Remove any stray <svg> tags if left by minification
        .pipe(replace(/<\/?svg(.*?)>/g, ''))
